...
*****
O `purrr` possui diversar funções, muitas delas não aperecam nesse post. Aqui focamos em explicar as funções que são mais utilizadas e algumas mais simples. As funções de `purrr` possuem uma sintaxe muito parecida, então se entender bem as funções que trouxemos aqui, as outras serão bem fáceis de entender. Vale lembrar que pode acesar o cheatsheets para ver todas as funções  clicando [aqui](https://github.com/rstudio/cheatsheets/blob/master/purrr.pdf).  
